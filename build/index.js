...
var _GeojsonEquality = class _GeojsonEquality2 {
  constructor(opts) {
    ...
    this.compareProperties = (_c = opts == null ? undefined : opts.compareProperties) != null ? _c : true;
  }
  compare(g1, g2) {
    if (g1.type !== g2.type) {
      return false;
  ...
  compareFeature(g1, g2) {
    return g1.id === g2.id && (this.compareProperties ? import_deep_equal.default(g1.properties, g2.properties) : true) && this.compareBBox(g1, g2) && this.compare(g1.geometry, g2.geometry);
  }
...
function mutatingRealChangesetElementParser(mutatingJson) {
  ...
  function createWay(data) {
    ...
    const geometry2 = data.nodes.filter(function(node) {
  ...
  function createRelation(data) {
    if ("members" in data) {
      ...
      const feature2 = turf_bbox_polygon_default(turf_bbox_default(featureCollection(data.relations)));
  ...
  return ("old" in mutatingJson ? [omit_default(["old"], mutatingJson), mutatingJson.old] : [mutatingJson]).map(createFeature);
}
var isClosedWay = function(nodes) {
  if (nodes.length > 3) {
    const firstNode = nodes[0];
    const lastNode = nodes[nodes.length - 1];
    return Object.keys(firstNode).includes("lat") && Object.keys(firstNode).includes("lon") && Object.keys(lastNode).includes("lat") && Object.keys(lastNode).includes("lon") && firstNode.lat === lastNode.lat && firstNode.lon === lastNode.lon;
  }
  return false;
};
